...
class OversegmentationStatistics(luigi.Task):
    ...
    def run(self):
        ...
        def extract_patch_statistics_slice(z):
            ...
            patches = nifty.tools.blocking( roiBegin = [0L,0L], roiEnd = [ny,nx], blockShape = self.patchSize )
        ...
        with concurrent.ThreadPoolExecutor(max_workers=PipelineParameter().nThreads) as executor:
            tasks = []
