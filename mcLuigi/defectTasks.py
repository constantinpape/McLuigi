...
class OversegmentationPatchStatistics(luigi.Task):
    ...
    def run(self):
        ...
        # TODO calculate histogram to have a closer look at the stats
        mean = np.mean(segs_per_patch)
        std  = np.std(segs_per_patch)
        histo, _ = np.histogram(segs_per_patch, bins = 32)

        stats = np.zeros([histo.shape[0]+2])
        stats[0] = mean
        stats[1] = std
        stats[2:] = histo
        ...
        def detect_patches_z(z):
            ...
            for patch_id in xrange(patches.numberOfBlocks):
                ...
                if n_segments < self.defectThreshold * mean_num_segs:
                    this_patch = np.ones( tuple(map(lambda x,y:x-y, patch_end, patch_end)), dtype=np.uint8)
