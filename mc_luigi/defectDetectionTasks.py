...
class OversegmentationSliceStatistics(luigi.Task):
    ...
    def output(self):
        ...
        save_path = os.path.join(
            ...
            "OversegmentationSliceStatistics_%s_nBins%i.h5" % (segFile, PipelineParameter().nBinsSliceStatistics)
        )
        return HDF5DataTarget(save_path)
    ...
    def run(self):
        ...
        def detect_defected_slice(z):
            ...
            if n_segs < threshold:
                ...
                return True
            # FIXME for some reason we have to write here,
            # otherwise there is strange behaviour when loading the data slice by slice
            else:
